...
for ($i = 0;$i < $cc;$i++) {
	// 更新用户过期定单 {{{
	// 查询所有下单但未购买的用户定单 status = 1 AND showStatus = 1
	...
	echo "expired order num ".$ret."\n";
	// }}}
	
	// 重新发布过期团购html {{{
	// 已经改好的团购id列表 {{{
	include_once WEB_ROOT.'shell/newGroupIds.php';
	// }}}
	...
	//if (in_array($groupArray[$i]["groupId"], $newGroupIds)){
	if (in_array($groupArray[$i]["groupId"], $newGroupIds) || $groupArray[$i]["groupId"] >= 261){
		$url = SW_URL_CMS."modules/product/handle/publicdetail_expiredV2.php?groupId=".$groupArray[$i]["groupId"]."&eName=".$eName;
	}
