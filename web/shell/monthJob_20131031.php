...
for($dayCount = 1; $dayCount <= $monthDay_count; $dayCount++){
  if($dayCount < 10){
    $dayCount_str = "2013-10-0".$dayCount;
  }else{
    $dayCount_str = "2013-10-".$dayCount;
  }
  //首页
  ...
  //全部
  $sql = "insert into html_schedule (cityId,showDate,groupIds,type) values(".$cityId.",'".$dayCount_str."','246,544',2)";
  echo $sql."\n";
  $mysql = mysql_connect(SW_DB_WRITE_HOSTS,SW_DB_WRITE_USER,SW_DB_WRITE_PASS);
  if (!mysql_ping($mysql)){
    echo "mysql_ping; \n";
...
for($dayCount = 1; $dayCount <= $monthDay_count; $dayCount++){
  ...
  //首页
  $sql = "insert into html_schedule (cityId,showDate,groupIds,type) values(".$cityId.",'".$dayCount_str."','285,245,366',1)";
  echo $sql."\n";
  $mysql = mysql_connect(SW_DB_WRITE_HOSTS,SW_DB_WRITE_USER,SW_DB_WRITE_PASS);
  if (!mysql_ping($mysql)){
    echo "mysql_ping; \n";
  ...
  //阅读
  $sql = "insert into html_schedule (cityId,showDate,groupIds,type) values(".$cityId.",'".$dayCount_str."','245',11)";
  echo $sql."\n";
  $mysql = mysql_connect(SW_DB_WRITE_HOSTS,SW_DB_WRITE_USER,SW_DB_WRITE_PASS);
  if (!mysql_ping($mysql)){
    echo "mysql_ping; \n";
