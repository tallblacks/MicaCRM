<?php
  ...
  if ($_POST){
    ...
    if($ordernum < 1 || $ordernum > 1000){
      $message="商品数量输入有误！";
  	  ...
  	  $isHave = $suborderManager->checkSuborder($orderid,$productid);
	    ...
	    $returnFlag = $suborderManager->create($new_suborder);
        if ($returnFlag){
	      $message="成功在订单中添加产品";
	      Header("Location:createsuborder.php?orderid=$orderid&msg=$message");
	      exit();
	    }else{
	      $message="在订单中添加产品失败";
	    }
	  }else{
	    $message="您已经在本订单中添加过该产品，请勿重复添加！";
	  }
  ...
  if(!$_POST){
    ...
    $rate = $order->getNRate();
  	...
  	if($ffree == 0){
  	  $ffree_str = "<input type=\"radio\" name=\"ffree\" value=\"0\" checked>不包
	       <input type=\"radio\" name=\"ffree\" value=\"1\">包邮";
  	}else{
  	  $ffree_str = "<input type=\"radio\" name=\"ffree\" value=\"0\">不包
	       <input type=\"radio\" name=\"ffree\" value=\"1\" checked>包邮";
  	}
  	...
  	if($_SESSION['SESS_TYPE'] == ADMIN){
  	  ...
  	  $spricecnStr2 = "<tr {$strings['TABLE_DARK_COLOR']}>
         ...
         <td >&nbsp;<input type=\"text\" name=\"price\" value=\"$spricecn\">可以在这里设定针对这个订单的人民币单价</td>
       </tr>";
  	}else{
  	  $spricecnStr1 = "<input type=\"hidden\" name=\"price\" value=\"$spricecn\">";
  	  $spricecnStr2 = "<tr {$strings['TABLE_DARK_COLOR']}>
...
echo "<table width=\"100%\"  border=\"0\" cellpadding=\"0\" cellspacing=\"1\" {$strings['TABLE_LINK_BK_COLOR']} class=eng>
      ...
      $spricecnStr1
       <tr {$strings['TABLE_TITLE_BKCOLOR']}>
        <td colspan=\"2\" height=\"30\" class=hdr>为订单添加产品信息</td>
       </tr>
       ...
       <tr {$strings['TABLE_DARK_COLOR']}>
         ...
         <td >&nbsp;<input type=\"text\" name=\"ordernum\" value=\"1\"></td>
       </tr>
    ...
    <tr{$strings['TABLE_DARK_COLOR']}>
     <td height=\"40\" colspan=\"2\" align=\"center\"><input type=\"submit\" name=\"Submit\" value=\"确认添加产品\"></td>
    </tr>
   </form>
   </table>";
?>